...
class EditTricountPage extends ConsumerStatefulWidget {

  const EditTricountPage({
    super.key
  });

  @override
  ConsumerState<EditTricountPage> createState() => _EditTricountPageState();
}

class _EditTricountPageState extends ConsumerState<EditTricountPage> {
  ...
  void initState() {
    ...
    _titleController = ValidatingTextEditingController(
      ...
      initialValue: tricount.title,  // pré-rempli
      asyncValidator: (s) => TricountValidator.validateTricountUnicity(
        title: s,
        tricountId: tricount.id // car modification de tricount
      ),
    ...
    _descriptionController = ValidatingTextEditingController(
      ...
      initialValue: tricount.description, // pré-rempli
  ...
  Widget data(
    ...
    if (participants.isEmpty) {
        participants.addAll(tricount.participants);
    }
  ...
  Widget _participantTile(User participant) {
    ...
    final bool isCreator = participant == tricount.creator;

    // Impliqué dans une dépense ?
    // (initiateur OU présent dans au moins une répartition)
    final bool isImplicatedInExpense = tricount.isInvolved(participant);

    // Peut-on le supprimer ?
    //  pas le créateur
    //  pas impliqué dans une dépense
    final bool isDeletable = !isCreator && !isImplicatedInExpense;
